...
public class BulkDataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        Integer botUserCount = jdbcTemplate.queryForObject(
    ...
    private void seedData() {
        try {
            ...
            List<Long> userIds = jdbcTemplate.queryForList("SELECT id FROM users WHERE email LIKE 'bot%@daypoo.sim'", Long.class);
            List<Long> toiletIds = jdbcTemplate.queryForList("SELECT id FROM toilets LIMIT 100", Long.class);
    ...
    private void seedUsers() {
        ...
        for (int i = 0; i < count; i++) {
            users.add(new Object[]{
                    "bot" + i + "@daypoo.sim",
                    botPass,
                    SeedDataGenerator.generateRandomName(),
                    0L, 0L, "ROLE_USER",
    ...
    private void seedItemsAndTitles() {
        ...
        items.add(new Object[]{"빛나는 똥", "최고의 명예", "HEAD", 1000L, null, Timestamp.valueOf(LocalDateTime.now()), Timestamp.valueOf(LocalDateTime.now())});
        items.add(new Object[]{"급한 휴지", "속도를 올린다", "HAND", 200L, null, Timestamp.valueOf(LocalDateTime.now()), Timestamp.valueOf(LocalDateTime.now())});
        insertHelper.bulkInsertItems(items);

        List<Object[]> titles = new ArrayList<>();
        titles.add(new Object[]{"똥쟁이", "첫 기록 달성", 1, 0L, Timestamp.valueOf(LocalDateTime.now())});
        titles.add(new Object[]{"변비 해결사", "기록 10회 달성", 5, 500L, Timestamp.valueOf(LocalDateTime.now())});
