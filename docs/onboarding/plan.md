...
- **소셜 로그인 (권장)**: 카카오, 구글 (3초 만에 시작하기). 복잡한 자체 가입 절차 대신 소셜 로그인을 주력으로 홍보.
...
- **테이블**: `TOILETS`, `POO_RECORDS`. (방문 기록은 POO_RECORDS로 통합)
...
- **인증 UI**: 화장실 반경(150m) 진입 시 '체크인' 및 '기록하기' 버튼 활성화. ('AI 간편 촬영' vs '수동 입력' 옵션 제공 및 사진 미저장/무음 고지 모달 포함)
- **체류 시간 검증**: 체크인 버튼 클릭 후 최소 1분이 지나야 최종 배변 기록 저장이 가능하도록 유도하는 UI 적용.
